...
7. [[funcmod-spatial-type]]The spatial model isrepresented by a nested grid structure comprising one or more grids.  To evaluate the spatial model at a specific position the nesting algorithm identifies which grid is applicable at that position and the disclocation is interpolated from this grid alone.  The grids are constrained to be:
 ...
 * a piecewise linear function of time defined by function values at ordered set of times.  The function is interpolated linearly between the specified values. It can be discontinuous – a step is created by two points with the same time and different multipiers.  Two specialisations of the piecewise linear time function are the step and reverse_step functions.
...
The time function is defined as the sum of one or more of the following generic functions.  In this table some functions include optional parameters start epoch and end epoch indicated by italics.

[cols="2,5a,5a"]
[options="header"]
|===
|Function type|Parameters|Formula (t~i,min~ &#8804; __t__ < t~i,max~)
|velocity| Reference epoch t~0~| f(_t_) = _t_ - t~0~

|piecewise|Array of corner points +
&nbsp;&nbsp;epoch t~i~ +
&nbsp;&nbsp;multiplier f~i~ +
for i=0..n|
f(__t__) = f~0~ for __t__ < t~0~

f(__t__) = (f~i~.(t~i+1~ – __t__) + f~i+1~.(__t__-t~i~))/(t~i+1~-t~i~) +
for t~i~ &#8804; __t__ < t~i+1~

f(__t__) = f~n~ for __t__ &#8805; t~n~

|exponential|Reference epoch t~0~ +
Start amplitude &#945;~0~ +
Final amplitude &#945;~∞~ +
Decay constant θ + 
_End epoch_ t~e~ | f(__t__) = 0     for __t__ < t~0~ 

f(__t__) = &#945;~0~ + (&#945;~∞~-&#945;~0~).(1 - exp(-(__t__-t~0~)/θ))     for __t__ &#8805; t~0~ +
f(_t_) = f(t~e~) for t &#8805; if t~e~ is defined

|logarithmic|Reference epoch t~0~ +
Scale factor &#945; +
Time factor &#964; + 
_End epoch_ t~e~| 0 for __t__ < t~0~ +
f(__t__) = &#945;.ln(1 + (__t__-t~0~)/&#964;) for t~0~ &#8804; __t__ +
f(_t_) = f(t~e~) for t &#8805; if t~e~ is defined

|acceleration|Reference epoch t~0~ +
Acceleration _a_ + 
_Start epoch_ t~s~ +
_end epoch_ t~e~|f(__t__)= _a_.(t - t~0~)^2^  +
f(_t_) = f(t~s~) for t < t~s~ if t~s~ is defined +
f(_t_) = f(t~e~) for t &#8805; if t~e~ is defined
...
Cosine amplitude &#945; +
Sine amplitude &#946;| 
f(__t__) = &#945;cos(_f_(__t__ - t~0~)/2&#120587;)
+ &#946;sin(_f_(__t__ - t~0~)/2&#120587;)
|===

To improve readability of the time function definition two common special cases are supported as a specialisations of the piecewise function:
...
f(__t__) = 1 when t &#8805; t~0~
|reverse_step | Reference epoch t~0~ |f(__t__) = -1 when t < t~0~, +
f(__t__) = 0 when t &#8805; t~0~
|===
