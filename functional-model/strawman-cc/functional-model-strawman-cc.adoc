...
&nbsp;&nbsp;multiplier f~i~ +
for i=0..n|
f(_t_) = f~0~ for _t_ < t~s~

f(_t_) = (f~i~.(t~i+1~ – _t_) + f~i+1~.(_t_-t~i~))/(t~i+1~-t~i~) +
for t~i~ &#8804; _t_ < t~i+1~

f(_t_) = f~n~ for _t_ &#8805; t~n~
...
|acceleration|Start epoch t~s~ +
Acceleration _a_ + 
_end epoch_ t~e~|f(_t_) = 0 if _t_ < t~s~ +
f(_t_)= _a_.(t - t~s~)^2^  if _t_ &#880t; t~s~ +
f(_t_) = f(t~e~) for _t_ > t~e~ if t~e~ is defined

|hyperbolic tangent|Start epoch t~s~ +
End epoch t~1~ +
Time constant &#964; +
Amplitude &#945;|f(_t_)= 0 for _t_ < t~s~ +
...
|step | Reference epoch t~s~ |f(__t__) = 0 when t < t~s~, +
f(__t__) = 1 when t &#8805; t~s~
|reverse_step | Reference epoch t~s~ |f(__t__) = -1 when t < t~s~, +
