...
|exponential|f(__t__) = f~p~     when __t__ < t~0~ 
...
For function type “exponential” the function is defined by a reference epoch t~0~, an optional end epoch t~1~, a relaxation constant θ, and three scale factors, before f~p~, initial f~0~, and final f~∞~.  
