...
* Distortion models used to represent the deformation between two datums (typically between older datums based on local terrestrial observations and modern datums based on global satellite systems)
...
8. [[funcmod-time-type]]The time function is the sum of one or more one of:
...
A simple average of the east and north displacements may not be appropriate very close to the poles where the directions of the east and north vectors may be quite different at the set of nodes being averaged, as shown in the figure where the directions at nodes A and B and the evaluation point P are all quite different.  
A latitude/longitude grid may not be appropriate for models including polar regions - for example it may be simpler to use a grid defined on suitable projected coordinate system.   See the discussion on  <<discuss-geocentric-interpolation, errors of normal bilinear interpolation near poles>> below.

[.right]
image::geocentric_bilinear_interpolation.png[Alt=geocentric bilinear interpolation diagram, width=200,scalewidth=7cm]

However a weighted average still services if the directions of the north and east vectors are accounted for in averaging the node values..  
...
For the uncertainties eh, ev the magnitude of the uncertainty is determined relative to the <<discuss-uncertainty-epoch, uncertainty reference epch>>. The scale factor to apply to undertainties is f~e~(__t__) is defined as  √abs(f(__t__)-f(t~0~)) where t~0~ is the uncertainty reference epoch of the model. 
...
[options="header"]
|===
|Function type|Parameters|Formula (t~min~ &#8804; __t__ &#8804; t~max~)
...
Note that the constant, velocity, step, and reverse step functions are all special cases of the piecewise linear function.  The piecewise linear function can 
include two consecutive time points at the same epoch (i.e. t~i~ = t ~i+1~) which creates a step.  
...
If the model includes a <<funcmod-14prm-transformation, 14 parameter transforamtion>> then this is applied to the coordinates before the 
deformation model is calculated or applied in a forward transformation.  In an inverse transformation it is applied (in reverse) after the deformation model components have been applied to the coordinate.
...
This means that the deformation model cannot exactly model deformation.  For example where deformation is due to surface faulting the actual deformation may not be continuous across a fault line.  
...
However the purpose of the deformation model in this context is not to exactly represent deformation, but to represent it to the extent that is useful within the context of coordinate transformations.  
...
The requirement for continuity does have implications for how models are defined.  For example it means that in nested grids child grids must be aligned with the parent grid as shown below to ensure continuity at the edge of the child grid.  Also it may limit the interpolation methods that could be used in nested grids - for example bicubic interpolation cannot ensure continuity at the edge of the child grid. (These constraints could be overcome if the child grid deformation is used by adding it to the parent grid deformation rather than replacing it - in effect the child grid becomes a separate deformation component).
...
Currently this specification takes the producers perspective - a transformation beyond the extents of the model should fail. However this is open for debate.
