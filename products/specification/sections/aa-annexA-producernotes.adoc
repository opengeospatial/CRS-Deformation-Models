...
This deformation model specification assumes that the deformation can be partitioned into one or more elements each of which is characterized by a time function.  There are many other ways deformation can be modeled.  For example in geophysical research it is often modeled using uniform dislocation on a rectangular fault surfaces in a uniform elastic half space.  While such models may be used to research and develop deformation models, the common practice in publishing them for coordinate transformations is to convert the model to a grid describing the resultant surface displacement.  
...
Within a single grid the use of bilinear interpolation ensures that the deformation is spatially continuous.  It also ensures the deformation is invertible provided the displacements calculated in the grid are not greater than the grid spacing, which is generally very unlikely.
...
Representing deformation in near polar regions using this specification may 
not give the desired results if the interpolation coordinate system or source and target coordinate systems are geographic. Both the means of interpolating across a grid cell, and the formulae for applying the displacment to a coordinate, may not be suitable.  The simplest way to represent deformation in such cases is to use a suitable projection coordinate system.  Future versions of this specification may add support for geographic coordinate systems in polar regions.

Near the poles grid cells may span a large longitude range. Consequently the east and north components of a vector at adjacent grid nodes may be in quite different directions, as illustrated in <<image-near-pole-axes>>.  Interpolating east and north components independently as described in <<formula-bilinear-interpolation>> fails to account for this. For example, if the grid cell spans 1° of longitude and the displacement is 1m then bilinear interpolation may give rise to an error of up to about 2cm (the length of the displacement vector multiplied by the cosine of the longitude range of the grid cell).  This can be mitigated by using a finer longitude grid spacing.
...
Using start and end epochs has practical value for deformation model producers managing a coordinate system. In particular using the start epoch means that displacements calculated before that epoch do not change.  This may be more realistic geophysically. Although the mathematical model does not define it, there is a time at which the slow slip event starts, and before which it is causing no deformation.
