...
Deformation models described in this specification are intended for use in coordinate operations - either to define time dependent transformations between different coordinate systems or to track the motion of objects between different epochs within a single coordinate system.  
...
The formulae defined in this specification are not suitable for transforming geographic (longitude and latitude) coordinates in near polar regions. Both the means of interpolating within a grid cell, and the formulae for applying the displacment to a coordinate, may yield unintended results.  The simplest way to represent deformation in such cases is to use a suitable projected (easting and northing) coordinate system.  Future versions of this specification may add support for geographic coordinate systems in polar regions.
