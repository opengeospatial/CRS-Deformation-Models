...
The formulae defined in this specification are not suitable for transforming geographic (longitude and latitude) coordinates in near polar regions. Both the means of interpolating within a grid cell, and the formulae for applying the displacement to a coordinate, may yield unintended results.  The simplest way to represent deformation in such cases is to use a suitable projected (easting and northing) coordinate system.  Future versions of this specification may add support for geographic coordinate systems in polar regions.
...
Using start and end epochs has practical value for deformation model producers managing a coordinate system. In particular using the start epoch means that displacements calculated before that epoch do not change.  This may be more realistic. Although the mathematical model does not define it, there is a time at which the slow slip event starts, and before which it is causing no deformation.
