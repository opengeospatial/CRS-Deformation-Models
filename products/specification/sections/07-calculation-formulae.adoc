...
The spatial function defines a set of parameters at each grid node which may be a subset of the parameters defined for the deformation model.  For example, the spatial function of one element define only horizontal displacements and horizontal uncertainties, and that of another element may define vertical displacements and uncertainties.  In order to combine all the elements to calculate to total displacement and uncertainty as described in <<formula-elements>>, a spatial function must define values for parameters that are not defined in the grids.

A parameter of horizontal or vertical displacement that are not defined as grid parameters are evaluated as zero for the spatial function. 
