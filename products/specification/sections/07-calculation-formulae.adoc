...
The time function is defined as the sum of one or more of the following base functions. In this table some functions include optional parameters which are indicated by italics.  The default value for optional parameters is shown in parentheses where a value is required.
...
_Scale factor_ s (1.0) +
_Start epoch_ t~s~ +
_End epoch_ t~e~ | f(_t_) = s.(_t_ - t~0~) +
f(_t_) = f(t~s~) for _t_ < t~s~ if t~s~ is defined +
f(_t_) = f(t~e~) for _t_ < t~e~ if t~e~ is defined

|step | Reference epoch t~0~ +
_Scale factor_ s (1.0) | f(__t__) = 0 when t < t~0~, +
f(__t__) = s when t &#8805; t~0~

|reverse step | Reference epoch t~0~ +
_Scale factor_ s (1.0) | f(__t__) = -s when t < t~0~, +
f(__t__) = 0 when t &#8805; t~0~

|ramp|Start epoch t~s~ +
End epoch t~e~ +
_Start scale factor_ s~s~ (0.0) +
_End scale factor_ s~e~ (1.0) |f(_t_) = s~s~ for _t_ < t~s~ +
f(_t_) = (s~s~.(t~e~ – _t_) + s~e~.(_t_-t~s~))/(t~e~-t~s~) for t~s~ &#8804; _t_ < t~e~ +
f(_t_) = s~e~ for _t_ &#8805; t~e~

|exponential|Start epoch t~s~ +
Decay constant θ +
_Scale factor_ s (1.0) +
_End epoch_ t~e~ | f(_t_) = 0   for _t_ < t~s~ +
f(_t_) = s.(1 - exp(-(_t_-t~s~)/θ))     for _t_ &#8805; t~s~ +
f(_t_) = f(t~e~) for _t_ > t~e~ if t~e~ is defined
...
_Scale factor_ s (1.0) +
_End epoch_ t~e~| f(_t_) = 0 for _t_ < t~s~ +
f(_t_) = s.ln(1 + (_t_-t~s~)/&#964;) for t~s~ &#8804; _t_ +
f(_t_) = f(t~e~) for _t_ > t~e~ if t~e~ is defined

|acceleration|Function reference epoch t~0~ +
_Scale factor_ s (1.0) +
_Start epoch_ t~s~ +
_End epoch_ t~e~|f(_t_)= s.(_t_ - t~0~)^2^  +
f(_t_) = f(t~s~) for _t_ < t~s~ if t~s~ is defined +
f(_t_) = f(t~e~) for _t_ > t~e~ if t~e~ is defined
...
End epoch t~1~ +
Time constant &#964; +
_Scale factor_ s (1.0) | f(_t_)= 0 for _t_ < t~s~ +
f(_t_) = s/2 + A.tanh((_t_ - t~r~)/&#964;) for _t_ &#8805; t~s~ and _t_ < t~e~ +
f(_t_) = s for _t_ &#8805; t~e~ +
where: +
t~r~ = (t~s~ + t~e~)/2 +
A = s/2.tanh((t~e~-t~r~)/&#964;) +
tanh(x) = (e^x^ - e^-x^)/(e^x^ + e^-x^)

|cyclic|Frequency _f_ (cycles per year) +
Function reference epoch t~0~ +
Cosine scale factor &#945; +
Sine scale factor &#946; |
...
Note that for calculating displacement components this gives the same result as taking the difference between the displacement components calculated individually for each epoch.  However for the calculating the uncertainty of the displacement between two epochs this method must be used to give the correct result.  
