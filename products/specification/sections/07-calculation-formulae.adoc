...
To improve simplicity and readability of the time function definition two common specialistations of the ramp function are provided for:
...
Note: a velocity function including a start and end epoch is functionally identical to a ramp function.  However these two options are provided to support quite different cases.  
...
The ramp function is to support a specific time bounded event.  It more meaningfully represents specific deformation events as the spatial model displacements reflect the total deformation rather than the rate of deformation.  Also it supports the step function specialisation which is not possible with a velocity function (as the velocity is infinite).  Multiple ramp functions can be combined to approximate any time evolution.
